---
name: "システムテスト項目書を作成・実施する"
description: "Playwrightテストコードからテスト項目書を生成し、テストを実行して結果を反映する"
---

# システムテスト項目書作成/テスト実行プロンプト

## 目的

既存の Playwright E2E テストコードを分析し、テスト項目書を生成したうえでシステムテストを実行し、結果をテスト項目書へ反映する

## ステップ概要

1. **Playwright テストコード確認** — `frontend\tests\e2e\` 配下の既存 `.spec.ts` を確認する
2. **システムテスト項目書作成** — テストコードを正本として Markdown のシステムテスト項目書を生成する
3. **テスト実施** — Playwright を実行する
4. **結果反映** — 実行結果をシステムテスト項目書の結果・備考へ反映する
...
- **結果**：テスト実施前は空欄、実施後は `OK` または `NG` を記載する
...
- 既存のテストコードを変更せず、テスト項目書作成と結果反映を優先する
