...
description: "設計書をもとにSTテスト観点を洗い出し、観点表（Markdown）を生成する。後続の試験項目書作成の材料となる。"
---
...
|------|--------------|----------------------|
| **業務シナリオの一貫性** | 登録→参照→更新→削除の一連の流れを通じてデータ・状態が整合しているか | 業務フローの途中・完了後に、画面・DB・メッセージが矛盾なく一致していること |
| **画面遷移** | 遷移先・遷移条件・ブラウザバック時の動作が正しいか | 設計書に定義された遷移先・条件どおりに画面が切り替わること |
| **メッセージ** | 入力・操作・画面遷移の各タイミングでメッセージが正しく表示されるか | 正常時・エラー時ともに適切なメッセージが、適切なタイミングで表示されること |
| **誤操作・例外操作** | 二重送信・連打・タイムアウト・セッション切れ・途中離脱時に適切に制御されるか | 想定外の操作に対してもシステムが安全な状態を保ち、適切なエラー案内が出ること |
| **権限・ロール** | ロールによる操作可否・表示制御が仕様どおりに機能しているか | 権限のないユーザが操作・閲覧できない状態になっていること |
| **外部連携** | API連携の正常・異常（失敗・タイムアウト）時に適切な挙動をするか | 連携先の障害時もユーザに適切なフィードバックがあり、システムが安定していること |
| **データ整合性** | 処理結果がDB・画面表示で矛盾していないか | 操作の前後でデータが意図どおりに更新・保持されていること |
| **セキュリティ** | 認証・認可が機能し、不正な操作・遷移が防止されているか | 認証なしの操作や権限外のリソースへのアクセスが遮断されていること |
| **ログ** | 操作・エラー・認証などの重要イベントが適切にログ出力されており、エラー発生時にエラー内容・エラーコード・操作の文脈が識別できる形で記録されているか | 必要なイベントがログに記録され、エラーの原因追跡が可能であること。また個人情報のマスキングが適用されていること |
