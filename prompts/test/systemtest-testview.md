# システム試験（ST）試験観点書 作成プロンプト

## 目的
ユーザの操作シナリオを起点に、抜け漏れのないST用の試験観点を網羅的に洗い出してください。
- 本プロンプトの成果物（**試験観点書**）は、後続の「**システム試験項目書**」を作成するための材料とする
- 単体・結合試験ではカバーしきれない、**ユーザの一連の操作シナリオ**に着眼した観点を抽出する
- 「何を確認すべきか（観点）」のレベルで記述し、具体的な手順・期待結果（項目書側で記述）までは踏み込みすぎない

## ステップ概要
1. **インプット資料の参照** - インプット資料の存在確認をし、システムの構成情報を取得する
2. **試験観点の抽出** - インプット資料及び記載の条件から試験観点を抽出する
3. **試験観点表の作成** - 抽出した観点をもとに観点表を.md形式で作成する。
...
### 2.1 着眼すべき観点の軸
最低限、以下の軸を考慮して観点の抜け漏れを防ぐこと。対象システムに該当しない軸は無理に含めない。
...
- **テスト観点**：小分類に属する試験項目を作成の際に基準とする観点
- **説明**：テスト観点の説明
...
###  3.2 制約・ルール
- 観点は曖昧表現（「正しく動くこと」のみ等）を避け、**どのような観点で試験項目を作成すればよいか**が分かるよう具体化する
- **入力材料に記載のない仕様を推測で補完しない**。
- 出力は **.md ファイル**（上記テーブル）として生成する
- 出力先は `docs\tests\system-test\items\` 配下とし、ファイル名は `systemtest-testview.md` とする



## 重要：不明点の確認
- 入力材料が不足している、または仕様が読み取れず観点を確定できない場合は、**推測で観点を作成せず、処理を止めて質問すること**。
