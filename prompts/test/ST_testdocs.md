...
- **No**：全体連番を 3 桁ゼロ埋めで採番すること（001〜）
- **テストID**：画面 ID と試験番号から一意に採番すること（例：`TC-LOGIN-001`, `TC-TOP-001`, `TC-DIVIDEND-001`, `TC-MYPAGE-001`）
- **試験区分**：`test()` 名の接頭辞や文言から `正常系 / 準正常系 / 異常系` を判定すること
- **試験項目**：確認観点を 1 行で簡潔に表現すること
- **試験手順**：手順番号付きで記載し、Markdown テーブル内では `<br>` 区切りを使用すること
- **期待結果**：`expect()` や URL 検証の内容を自然言語で記載すること
- **判定基準**：OK / NG を判断できる観測条件を明記すること
- **結果**：試験実施前は空欄、実施後は `OK` または `NG` を記載すること
- **備考**：API モック条件、テストデータ、前提画面、失敗原因などを記載すること

### 生成ルール

1. `describe` 単位でセクションを分けること
2. テストコードに存在しない試験項目は追加しないこと
3. 1 つの `test()` から 1 行の試験項目を生成すること
4. `test.step()` の文言は可能な限りそのまま試験手順へ使うこと
5. `saveEvidence()` は証跡取得処理として扱い、単独では試験手順にしないこと
6. `mockAppApi()` の差分設定がある場合は、備考欄へ API 応答条件を記載すること
7. `toHaveURL()`、`toBeVisible()`、`toContainText()`、`toBeDisabled()`、`toHaveValue()` などの検証内容を期待結果と判定基準へ反映すること
...
- `frontend/playwright.config.ts` の `baseURL` を使用すること
- Playwright project は `chromium` で実行すること
- ブラウザチャネル設定は `chrome` を前提とすること
...
1. 各試験項目の結果列に `OK` または `NG` を記載すること
2. テストが失敗した場合は、失敗した試験項目の備考欄へ原因を簡潔に記載すること
3. テストが成功した場合は、既存の備考を維持しつつ失敗理由を追記しないこと
4. 実行対象外の項目は作成しないこと
5. 試験結果の反映後も、`No` と `テストID` の採番を崩さないこと

---

## 6. 注意事項

- 推測で画面仕様や試験観点を補完しないこと
- テストコードに存在しないケースは追加しないこと
- 個人情報は必要最小限のテストデータ表現に留めること
- 既存のテストコードを変更せず、試験項目書作成と結果反映を優先すること
- `【要記入】` のようなプレースホルダは最終成果物に残さないこと
- 実行に失敗した場合は、その事実を試験項目書へ反映し、原因を備考欄へ記載すること
