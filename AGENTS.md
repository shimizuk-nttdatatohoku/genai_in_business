...
このファイルは、プロジェクト固有の事実情報（技術スタック、セットアップ、実行コマンド、ディレクトリ構造、環境変数、テスト実行方法）の正本として扱います。
...
| バックエンド | Python + FastAPI | 本番環境：`backend/requirements.txt` 開発環境：`backend/requirements-dev.txt` |
